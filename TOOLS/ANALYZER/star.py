...
class Star:
    ...
    def GrabPhotometry(self, keyword, value):
        # keyword is a two-character string (e.g., PB)
        color = keyword[1]
        words = value.split('|')
...
def ReadCatalog(target_name):
    ...
    try:
        with open(cat_path, 'r') as fp:
            ...
            return [Star(x) for x in fp.readlines() if len(x.strip()) > 25 and x.strip()[0] != '#']
    except IOError as x:
...
class IStar:
    def __init__(self, inst_mag_measurement, is_stacked_image, image_ref):
        ...
        self.gstar = next((x for x in context.catalog if x.name == self.name), None)
        if self.gstar is None:
            print("ERROR: IStar: name mismatch?? '"+self.name+"'", flush=True)
